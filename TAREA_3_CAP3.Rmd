...
ggplot() +
  geom_line(aes(x, y), color = "lightblue3", linewidth = 1.5) +
  geom_function(fun = fv_den, color = "red", linewidth = 0.5) +
  xlim(-1.5, 1.5) +
...
decision <- function(Vj, Vi, B = 1) {
  ...
  ggplot() +
    geom_function(fun = fv_den, color = "dodgerblue", linewidth = 1.5, xlim = c(-B - 0.1, B + 0.1)) +
